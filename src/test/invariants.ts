  ...
  if (node.left !== null) {
    ...
    if (keysEqual) {
      if (tree.onDuplicateKeys !== ResolveAction.Insert) {
        throw new Error(
          `Child on the left has the same value of its parent ${node.value} while duplicates are not allowed`
        );
      }
    } else if (!tree.isKeyLessThan(leftKey, key)) {
      throw new Error(
        `Child on the left of ${node.value} is not strictly smaller than its parent.`
      );
    }
  }
  if (node.right !== null) {
    ...
    if (keysEqual) {
      if (tree.onDuplicateKeys !== ResolveAction.Insert) {
        throw new Error(
          `Child on the right has the same value of its parent ${node.value} while duplicates are not allowed`
        );
      }
    } else if (!tree.isKeyLessThan(key, rightKey)) {
      throw new Error(
        `Child on the right of ${node.value} is not strictly larger than its parent.`
      );
    }
  }
}
...
export function checkAVLTreeInvariants<T, K>(this: AVLTreeIndex<T, K>) {
  const visit = (node: AVLNode<T> | null): number => {
    ...
    if (rightHeight - leftHeight !== node.balance) {
      throw new Error(
        `Node with value ${node.value} has a balance of ${
          node.balance
        } while it should be ${rightHeight - leftHeight}`
      );
    }
  ...
  const visit = (node: RBNode<T> | null): number => {
    ...
    if (node.color === RBColor.Red) {
      if (left !== null && left.color === RBColor.Red) {
        throw new Error(
          `Node on the left of node with value ${node.value} is red while the parent is also red.`
        );
      }
      if (right !== null && right.color === RBColor.Red) {
        throw new Error(
          `Node on the right of node with value ${node.value} is red while the parent is also red.`
        );
      }
    }
    ...
    if (blackCountLeft !== blackCountRight) {
      throw new Error(
        `The amount of black nodes on the left of node with value ${node.value} does not equal the amount of black nodes on the right`
      );
    }
