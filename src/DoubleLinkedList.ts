...
export class DLNode<T> extends CursorBase<T> {
  constructor(
    public value: T,
    public _prevNode: DLNode<T> | null = null,
    public _nextNode: DLNode<T> | null = null
  ) {
    super();
  }
...
export class DoubleLinkedListRange<T> extends RangeBase<T> {
  constructor(
    protected _startNode: DLNode<T> | null,
    protected _endNode: DLNode<T> | null,
  ...
  public reverse(): Range<T> {
    return new DoubleLinkedListRange(
      this._endNode,
      this._startNode,
      !this.reversed
    );
    ...
    while (position > 0) {
      ...
      if (node === null) {
        throw new RangeError(
          `Cannot get element at position ${position}: index out of bounds.`
        );
      }
      --position;
    }
